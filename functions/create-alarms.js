...
const enableDetailedMetrics = async (restApiId, stageName) => {
  ...
  const isDetailedMetricsEnabled = _.get(getResp, 'methodSettings.*/*.metricsEnabled', false);
...
const createAlarmsForEndpoints = async (restApiId, stageName) => {
  ...
  await restEndpoints.forEach(endpoint => {
    const putReq = {
...
module.exports.handler = async (event, context, cb) => {
  const { restApiId } = event.detail.requestParameters;
  const { stageName } = event.detail.requestParameters.createDeploymentInput;
